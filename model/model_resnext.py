...
class ResNeXt(nn.Module):
    ...
    def __init__(self,block,layers,sample_size, sample_duration,shortcut_type='B',cardinality=32,num_classes=2):
        self.name ='require naming model '
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks,shortcut_type,cardinality, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if shortcut_type == 'A':
                ...
                downsample = nn.Sequential(nn.Conv3d(self.inplanes,planes * block.expansion,kernel_size=1,stride=stride,
...
def downsample_basic_block(x, planes, stride):
    ...
    out = Variable(torch.cat([out.data, zero_pads], dim=1), requires_grad = True)
